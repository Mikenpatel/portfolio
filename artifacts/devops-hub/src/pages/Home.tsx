...
export default function Home() {
  const totalIncidents = TOOLS.reduce((acc, t) => 
    acc + t.metrics.filter(m => m.label.toLowerCase().includes("incident")).reduce((sum, m) => sum + m.count, 0)
  , 0);

  const totalRITMs = TOOLS.reduce((acc, t) => 
    acc + t.metrics.filter(m => m.label.toLowerCase().includes("ritm")).reduce((sum, m) => sum + m.count, 0)
  , 0);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <motion.div 
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-bold tracking-tight text-foreground mb-2">Systems Overview</h1>
        <p className="text-muted-foreground">Monitor and manage internal DevOps infrastructure and tools.</p>
      </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs pt-4 border-t border-border">
                <span className="text-muted-foreground">{tool.instances.length} Instance{tool.instances.length !== 1 && 's'}</span>
